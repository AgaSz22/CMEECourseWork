...
for (unique_id in ids){
  ...
  if (nrow(sub_df) < 7) { # if there are fewer datapoints than 7 (average no. of parameters in models), terminate loop
    ...
    aicc_logistic <-  AICc(fit_logistic, return.K = FALSE, second.ord = TRUE, 
    ...
    values_for_ot <- c(model = "logistic model",
                       unique_id,
    ...
    output_table = rbind(output_table, values_for_ot)
    
    colnames(output_table)[1] <- "Model"
    colnames(output_table)[2] <- "Unique ID"
    colnames(output_table)[3] <- "N_0"
    colnames(output_table)[4] <- "K"
    colnames(output_table)[5] <- "r_max"
    colnames(output_table)[6] <- "Temperature"
    colnames(output_table)[7] <- "Medium"
    colnames(output_table)[8] <- "Species"
    colnames(output_table)[9] <- "Population size"
    colnames(output_table)[10] <- "Experiment"
    colnames(output_table)[11] <- "AICc"
    colnames(output_table)[12] <- "R-squared"
    
    ######### Creating an output table for the quadratic model
    
    values_for_ot_quadratic <- c(model = "quadratic model",
                                 unique_id,
                                 fit_quadratic[[1]][[1]],
                                 fit_quadratic[[1]][[2]],
                                 fit_quadratic[[1]][[3]],
    ...
    colnames(output_table_quadratic)[1] <- "Model"
    colnames(output_table_quadratic)[2] <- "Unique ID"
    colnames(output_table_quadratic)[3] <- "Intercept"
    colnames(output_table_quadratic)[4] <- "a"
    colnames(output_table_quadratic)[5] <- "b"
    ...
    colnames(output_table_quadratic)[9] <- "Population size"
    colnames(output_table_quadratic)[10] <- "Experiment"
    colnames(output_table_quadratic)[11] <- "AICc" 
    colnames(output_table_quadratic)[12] <- "R-squared"
    
    ######### Creating an output table for the qubic model  
    
    values_for_ot_qubic <- c(model = "Qubic model",
                             unique_id,
                             fit_qubic[[1]][[1]],
                             fit_qubic[[1]][[2]],
                             fit_qubic[[1]][[3]],
    ...
    colnames(output_table_qubic)[1] <- "Model"
    colnames(output_table_qubic)[2] <- "Unique ID"
    colnames(output_table_qubic)[3] <- "Intercept"
    colnames(output_table_qubic)[4] <- "a"
    colnames(output_table_qubic)[5] <- "b"
    colnames(output_table_qubic)[6] <- "c"
    colnames(output_table_qubic)[7] <- "Temperature"
    colnames(output_table_qubic)[8] <- "Medium"
    colnames(output_table_qubic)[9] <- "Species"
    colnames(output_table_qubic)[10] <- "Population size"
    colnames(output_table_qubic)[11] <- "Experiment"
    colnames(output_table_qubic)[12] <- "AICc"
    colnames(output_table_qubic)[13] <- "R-squared"
