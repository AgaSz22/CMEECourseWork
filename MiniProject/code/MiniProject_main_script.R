...
pdf(file = "../results/population_growth_models.pdf")
...
for (unique_id in ids){
  ...
  if (nrow(sub_df) < 7) { # if there are fewer data points than 7, terminate loop - that prevents AICc values to go into infinity
    ...
    shapiro_outcome_cubic<- shapiro.test(residuals_cubic)
    ...
    colnames(output_table_cubic)[9] <- "Shapiro_cubic"
    ...
    plot_models <-  ggplot(sub_df, aes(x = Time, y = LogPopBio)) +
      ...
      ggtitle(paste("ID =", unique_id))
    
    print(plot_models)
    
    print("stuff was plotted") ### Message for myself to see whether R is plotting my models
...
output_table <- merge(x = output_table_logistic, y =output_table_quadratic, by = "Unique_ID", all = TRUE)
output_table_full <- merge(x = output_table, y =output_table_cubic, by = "Unique_ID", all = TRUE) 

### Saving the data frame as a csv output file
write.csv(output_table_full,"../results/CMEE_miniproject_output", row.names = FALSE)
...
g <- ggplot(data_violin, aes(x=Model_type, y=AICc_value, fill=Model_type), group = Model_type) +
 ...
 g <- g + theme_classic()
 g <- g + theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14)) ### increasing x and y axis labels
 g <- g + theme(axis.title.y=element_text(size=14), axis.title.x=element_text(size=14)) ### increasing x and y axis title
 g <- g + theme(legend.position = "bottom", 
                                    plot.title = element_text(hjust = 0.5),legend.title = element_text( size=15), legend.text = element_text(size = 14)) ### increasing legend title and text
